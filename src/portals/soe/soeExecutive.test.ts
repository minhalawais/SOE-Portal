...
describe('SOE Executive intelligence', () => {
  it('uses a dedicated organization-scoped, read-only experience', () => {
    expect(getHomeForRole(ROLE.SOE_EXECUTIVE)).toBe('/soe-review/executive')
    ...
    const definition = getPortalDefinitionForRole(PORTAL.SOE_REVIEW, ROLE.SOE_EXECUTIVE)
    expect(definition.homeRoute).toBe('/soe-review/executive')
