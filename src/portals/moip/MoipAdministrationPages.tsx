...
export function MoipUserManagementPage() {
  ...
  return <div>
    <PageHeader title="User management" subtitle="Identity lifecycle, roles, access scopes, MFA, sessions, invitations and account activity" actions={<Button size="sm" onClick={() => setShowInvite((value) => !value)}><UserPlus size={16} />Invite user</Button>} />
    <div className="mb-4 rounded-control border border-[#b8d6e8] bg-[#eff8fc] px-4 py-3 text-sm text-soe-ink"><strong>Credential security:</strong> administrators issue one-time activation or reset links. Passwords cannot be viewed, copied or recovered.</div>
    {showInvite ? <Card className="mb-4" title="Invite a user" subtitle="Assign predefined roles and optional custom module permissions."><div className="grid gap-4 xl:grid-cols-2">
      <div className="grid gap-3 sm:grid-cols-2"><TextField label="Full name" required value={name} onChange={(e) => setName(e.target.value)} /><TextField label="Official email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} /><TextField label="Ministry scope" value={ministry} onChange={(e) => setMinistry(e.target.value)} /><TextField label="Department scope" value={department} onChange={(e) => setDepartment(e.target.value)} /><DateField label="Temporary access expiry" value={temporaryAccessUntil} onChange={(e) => setTemporaryAccessUntil(e.target.value)} /></div>
      <div>
        <p className="mb-2 text-xs font-semibold uppercase text-soe-slate">Roles</p>
        <div className="grid gap-2 sm:grid-cols-2">
          {accountRoleOptions.map((item) => (
            <CheckboxField
              ...
              onChange={() => setRoles((current) => current.includes(item) ? current.filter((value) => value !== item) : [...current, item])}
            />
          ))}
          <CheckboxField
            label="Custom"
      ...
      <div className="xl:col-span-2"><p className="mb-2 text-xs font-semibold uppercase text-soe-slate">SOE scope</p><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{organizations.data?.items.map((item) => <CheckboxField key={item.id} label={`${item.abbreviation} · ${item.name}`} checked={organizationIds.includes(item.id)} onChange={() => setOrganizationIds((current) => current.includes(item.id) ? current.filter((value) => value !== item.id) : [...current, item.id])} />)}</div></div>
      <div className="flex gap-2"><Button disabled={!canInvite} loading={inviteMutation.isPending} onClick={() => inviteMutation.mutate()}>Send activation link</Button><Button variant="tertiary" onClick={() => setShowInvite(false)}>Cancel</Button></div>
    </div></Card> : null}
    <div className="mb-4 max-w-md"><TextField label="Search users" value={search} placeholder="Name, email, role or status" onChange={(e) => setSearch(e.target.value)} /></div>
    {users.isLoading ? <LoadingBlock /> : users.isError ? <ErrorState title="Unable to load users" /> : items.length ? <section className="overflow-hidden rounded-card border border-soe-border bg-white"><div className="overflow-x-auto"><table className="w-full min-w-[1420px] text-left text-sm"><thead className="bg-soe-canvas text-xs uppercase text-soe-slate"><tr><th className="px-4 py-3">User</th><th>Roles</th><th>SOE / portfolio scope</th><th>Ministry / department</th><th>Status</th><th>MFA</th><th>Last login</th><th>Failed</th><th>Sessions</th><th>Invitation / expiry</th><th>Actions</th></tr></thead><tbody>{items.map((user) => <tr key={user.id} className="border-t border-soe-border align-top"><td className="px-4 py-3"><Link className="font-semibold text-soe-blue hover:underline" to={`/moip/admin/users/${user.id}`}>{user.name}</Link><p className="text-xs text-soe-slate">{user.email}</p></td><td>{formatAssignedRoles(user.roles, user.customRoleEnabled, (item) => ROLE_LABEL[item as RoleId] ?? item)}</td><td>{user.organizationIds.length ? user.organizationIds.map(orgName).join(', ') : 'Portfolio scope'}</td><td>{user.ministryScopes.join(', ') || 'None'}<p className="text-xs text-soe-slate">{user.departmentScopes.join(', ') || 'No department scope'}</p></td><td><StatusBadge status={user.status} label={user.status.replaceAll('_', ' ')} /></td><td>{user.mfaEnabled ? <span className="inline-flex items-center gap-1 text-soe-teal"><ShieldCheck size={15} />Enabled</span> : 'Not enrolled'}</td><td>{formatDate(user.lastLoginAt)}</td><td>{user.failedLoginCount}</td><td>{user.activeSessions.length}</td><td className="capitalize">{user.invitationStatus}<p className="text-xs text-soe-slate">{formatDate(user.invitationExpiresAt)}</p></td><td><div className="flex gap-1"><Link className="inline-flex h-8 items-center gap-1 rounded-control px-2.5 text-xs font-medium text-soe-blue hover:bg-soe-canvas" to={`/moip/admin/users/${user.id}`}><Search size={14} />Manage</Link><Button variant="tertiary" size="sm" title="Send secure password-reset link" onClick={() => resetMutation.mutate(user.id)}><KeyRound size={14} />Reset</Button></div></td></tr>)}</tbody></table></div></section> : <EmptyState title="No matching user accounts" />}
  </div>
}

export function MoipAdministrationAuditPage() {
  const role = useSessionStore((state) => state.role)
  const query = useQuery({ queryKey: ['administration-audit'], queryFn: () => mockAdministrationService.listAuditEvents(role) })
  return <div><PageHeader title="Administrative activity" subtitle="SOE registry, user identity, access and lifecycle changes" />{query.isLoading ? <LoadingBlock /> : query.isError ? <ErrorState title="Unable to load administrative activity" /> : query.data?.length ? <Card><ol className="divide-y divide-soe-border">{query.data.map((event) => <li key={event.id} className="grid gap-1 py-3 text-sm sm:grid-cols-[180px_1fr_auto]"><p className="font-medium capitalize text-soe-navy">{event.action.replaceAll('_', ' ')}</p><p>{event.detail}</p><p className="text-xs text-soe-slate">{formatDate(event.occurredAt)} · {ROLE_LABEL[event.actorRole]}</p></li>)}</ol></Card> : <EmptyState title="No administrative events yet" hint="SOE and account changes will appear here." />}</div>
}
